...
def _protect_includes(match: re.Match) -> str:
    """
    Заменяет директиву препроцессора на защищенный маркер
    Args:
        match (Объект совпадения с директивой препроцессора)
    Return:
        f'__PROTECTED_INCLUDE_{len(protected_includes) - 1}__'
    """
    protected_includes.append(match.group())
    return f'__PROTECTED_INCLUDE_{len(protected_includes) - 1}__'


def _protect_strings(match: re.Match) -> str:
    """
    Заменяет строковый литерал на защищенный маркер
    Args:
        match (Объект совпадения со строковым литералом)
    Return:
        f'__PROTECTED_STRING_{len(protected_strings) - 1}__'
    """
    protected_strings.append(match.group())
    return f'__PROTECTED_STRING_{len(protected_strings) - 1}__'


def _restore_elements(code: str) -> str:
    """
    Восстанавливает директивы препроцессора и строковые литералы из маркеров
    Args:
        code (C++ код)
    Return:
        code
...
def _clear_variables() -> None:
    """
    Сбрасывает все глобальные переменные в исходное состояние
    Args:
        None
    Return:
        None
...
def _to_process_errors(operation_res: list) -> None:
    """
    Выводит результаты выполнения каждого шага обфускации
    Args:
        operation_res (Список результатов выполнения шагов обфускации)
    Return:
        None
    """
    all_ok = True
...
def _get_declarations_from_node(
    ...
    Args:
        node (узел AST),
        file_name (имя файла для проверки принадлежности),
        func_args (глобальный словарь функций по количеству аргументов),
        func_args_temp (временный словарь функций по количеству аргументов)
    Return:
        variables (переменные из текущего узла и его потомков),
        functions (функции из текущего узла и его потомков),
        classes (классы из текущего узла и его потомков)
    """
    variables = set()
    functions = list()
    classes = set()
...
def _parse_ast(file_name: str) -> tuple:
    ...
    Args:
        file_name (Путь к файлу для парсинга)
    Return:
        all_vars (все переменные из файла),
        all_funcs (все функции из файла),
        all_classes (все классы из файла),
        func_args_temp (Cловарь функций по количеству аргументов)
    """
...
def _filter_variables(all_vars: set, code: str) -> set:
    ...
    Args:
        all_vars (Множество всех переменных из AST),
        code (C++ код)
    Return:
        final_vars (Отфильтрованные имена переменных)
    """
    final_vars = set()
...
def _filter_functions(
    ...
    Args:
        all_funcs (Список всех функций из AST),
        func_args_temp (Cловарь функций по количеству аргументов),
        code (C++ код)
    Return:
        final_funcs (Отфильтрованные имена функций)
    """
...
def _filter_classes(all_classes: set, final_funcs: list) -> set:
    ...
    Args:
        all_classes (Множество всех классов из AST),
        final_funcs (Отфильтрованные функции)
    Return:
        final_classes (Отфильтрованные имена классов)
    """
    final_classes = set()
...
def _add_to_reserved(
    ...
    Args:
        final_vars (Отфильтрованные переменные),
        final_funcs (Отфильтрованные функции),
        final_classes (Отфильтрованные классы)
    Return:
        None
    """
    for i in final_vars:
